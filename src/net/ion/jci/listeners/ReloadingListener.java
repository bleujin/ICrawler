...
import net.ion.jci.stores.ResourceStore;
import net.ion.jci.stores.Transactional;
import net.ion.jci.utils.ConversionUtils;
 ...
 * or resources.
 */
public class ReloadingListener extends AbstractFileAlterationListener {
    ...
    public boolean isReloadRequired( final FileAlterationObserver pObserver ) {
        ...
        if (deleted.size() > 0) {
            for (File file : deleted) {
                final String resourceName = ConversionUtils.getResourceNameFromFileName(ConversionUtils.relative(pObserver.getDirectory(), file));
    ...
    void notifyReloadNotificationListeners() {
        for (ReloadNotificationListener listener : notificationListeners) {
            Debug.debug("notifying listener " + listener);
